...
func TestPersistentErrorDetection(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mmErr := NewMintMakerError(tt.errId, tt.nestedError)
			if mmErr.IsPersistent() != tt.isPersitent {
				t.Errorf("Wrong error persistance")
			}

			if mmErr.GetErrorId() != int(tt.errId) {
...
func TestNestedErrorMessage(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mmErr := NewMintMakerError(EUnknownError, tt.nestedError)
			mmErr.ExtraInfo = tt.extraInfo
			if mmErr.Error() != tt.expectedNestedErrorMessage {
				t.Errorf("Expected \"%s\" error message, but got \"%s\"", tt.expectedNestedErrorMessage, mmErr.Error())
			}
		})
	}
}
...
func TestShortErrorMessage(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mmErr := NewMintMakerError(tt.errId, tt.nestedError)
			if mmErr.ShortError() != tt.expectedShortMessage {
				t.Errorf("Expected \"%s\" error message, but got \"%s\"", tt.expectedShortMessage, mmErr.Error())
			}
		})
	}
}
