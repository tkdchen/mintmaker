...
import (
	...
	"github.com/konflux-ci/mintmaker/pkg/mmerrors"
)
...
func newGithubClientByApp(appId int64, privateKeyPem []byte, repoUrl string) (*GithubClient, error) {
	...
	if err != nil {
		...
		return nil, mmerrors.NewMintMakerError(mmerrors.EGitHubAppMalformedPrivateKey, err)
	}
	...
	for installId == 0 {
		...
		if err != nil {
			if resp != nil && resp.Response != nil && resp.Response.StatusCode != 0 {
				...
				case 401:
					return nil, mmerrors.NewMintMakerError(mmerrors.EGitHubAppPrivateKeyNotMatched, err)
				case 404:
					return nil, mmerrors.NewMintMakerError(mmerrors.EGitHubAppDoesNotExist, err)
				}
			}
			return nil, mmerrors.NewMintMakerError(mmerrors.ETransientError, err)
		}
	...
	if installId == 0 {
		...
		return nil, mmerrors.NewMintMakerError(mmerrors.EGitHubAppNotInstalled, err)
	}
	...
	if err != nil {
		if strings.Contains(err.Error(), "installation has been suspended") {
			return nil, mmerrors.NewMintMakerError(mmerrors.EGitHubAppSuspended, err)
		} else {
			return nil, err
		}
	}
...
func (g *GithubClient) GetConfiguredGitAppName() (string, string, error) {
	...
	if err != nil {
		// Inability to create transport based on a private key indicates that the key is bad formatted
		return "", "", mmerrors.NewMintMakerError(mmerrors.EGitHubAppMalformedPrivateKey, err)
	}
	client := github.NewClient(&http.Client{Transport: itr})
...
func getAppInstallations(githubAppIdStr string, appPrivateKeyPem []byte) ([]ApplicationInstallation, string, error) {
	...
	if err != nil {
		return nil, "", mmerrors.NewMintMakerError(mmerrors.EGitHubAppMalformedId,
