...
func refineGitHostingServiceError(response *http.Response, originErr error) error {
	...
	if _, ok := originErr.(*github.RateLimitError); ok {
		return mmerrors.NewMintMakerError(mmerrors.EGitHubReachRateLimit, originErr)
	}
	switch response.StatusCode {
	case http.StatusUnauthorized:
		// Client's access token can't be recognized by GitHub.
		return mmerrors.NewMintMakerError(mmerrors.EGitHubTokenUnauthorized, originErr)
	case http.StatusNotFound:
		...
		err := mmerrors.NewMintMakerError(mmerrors.EGitHubNoResourceToOperateOn, originErr)
		if len(scopes) == 0 {
...
func (g *GithubClient) branchExist(owner, repository, branch string) (bool, error) {
	...
	case 401:
		return false, mmerrors.NewMintMakerError(mmerrors.EGitHubTokenUnauthorized, err)
	case 404:
		return false, nil
	}
...
func (g *GithubClient) filesExistInDirectory(owner, repository, branch, directoryPath string, files []gp.RepositoryFile) ([]gp.RepositoryFile, error) {
	...
	if err != nil {
		switch resp.StatusCode {
		case 401:
			return existingFiles, mmerrors.NewMintMakerError(mmerrors.EGitHubTokenUnauthorized, err)
		case 404:
...
func (g *GithubClient) deleteWebhook(owner, repository string, webhookId int64) error {
	...
	if err != nil {
		switch resp.StatusCode {
		case 401:
			return mmerrors.NewMintMakerError(mmerrors.EGitHubTokenUnauthorized, err)
		case 404:
