...
import (
	...
	"github.com/konflux-ci/mintmaker/pkg/git/gitlab"
	"github.com/konflux-ci/mintmaker/pkg/git/gitprovider"
...
func createGitClient(gitClientConfig GitClientConfig) (gitprovider.GitProviderClient, error) {
	...
	case "github":
		if !isAppUsed {
			...
			if sshKeyExists {
				return nil, mmerrors.NewMintMakerError(mmerrors.EGitHubSecretTypeNotSupported,
		...
		if gitClientConfig.IsAppInstallationExpected {
			...
			if !appInstalled {
				return nil, mmerrors.NewMintMakerError(mmerrors.EGitHubAppNotInstalled,
	...
	case "gitlab":
		...
		if sshKeyExists {
			return nil, mmerrors.NewMintMakerError(mmerrors.EGitLabSecretTypeNotSupported,
	...
	case "bitbucket":
		return nil, mmerrors.NewMintMakerError(mmerrors.EUnknownGitProvider, fmt.Errorf("git provider %s is not supported", gitProvider))
	default:
		return nil, mmerrors.NewMintMakerError(mmerrors.EUnknownGitProvider, fmt.Errorf("git provider %s is not supported", gitProvider))
	}
}
